  ...
  try {
    ...
    if (!trades || trades.length === 0) {
      return NextResponse.json({
        ...
        profit_factor: 0, tp_rate: 0, sl_rate: 0, avg_duration: 0,
      })
    }

    // Calculate stats
    const wins = trades.filter(t => t.is_win === true)
    const losses = trades.filter(t => t.is_win === false)
    ...
    const tpHits = trades.filter(t => t.exit_reason === 'tp').length
    ...
    return NextResponse.json({
      ...
      tp_rate: (tpHits / trades.length) * 100,
